...
export const parseAutosaveProfile = async (filePath: string) => {
	...
	const servers = new Map<string, ParserServer>();
	const hosts = new Map<string, ParserHost>();
	const beacons = new Map<string, ParserBeacon>();
	const operators = new Map<string, ParserOperator>();
	const commands = new Map<string, ParserCommand>();
	const links = new Map<string, ParserLink>();

	for (const [server] of Object.entries(listeners)) {
		servers.set(server, { name: server, parsingPath: filePath });
	}

	for (const [beaconName, beaconInfo] of Object.entries(badgers)) {
		hosts.set(beaconInfo.b_h_name, { name: beaconInfo.b_h_name, server: beaconInfo.b_c2_id });
		beacons.set(beaconName, {
			name: beaconName,
	...
	for (const [operatorName] of Object.entries(users.active)) {
		operators.set(operatorName, { name: operatorName });
	}
	...
	for (const [user, userCommands] of Object.entries(user_activity)) {
		for (const [commandName, commandList] of Object.entries(userCommands)) {
			for (const command of commandList) {
				const currentBeacon = beacons.get(command.badger);
				...
				if (currentCommand) {
					...
					if (currentBeacon) {
						...
						beacons.set(currentBeacon.name, { ...currentBeacon, startTime, endTime });
					}
					commands.set(`${commandName}-${command.host}-${command.badger}-${command.time}`, {
						attackIds:
							(command.mitre
								?.flatMap((mitre) => [
	...
	const handleLinkChildren = (child: GraphBadger) => {
		if ('children' in child && child.children) {
			for (const childChild of child.children) {
				links.set(`${child.name}-${childChild.name}`, { from: child.name, to: childChild.name });
	...
	for (const [serverName, children] of Object.entries(badger_graph)) {
		for (const child of children) {
			links.set(`${serverName}-${child.name}`, { from: serverName, to: child.name });
