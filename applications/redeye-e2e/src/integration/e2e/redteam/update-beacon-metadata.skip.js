...
describe('Update Beacon metadata', () => {
	...
	it('Rename a beacon and change time of death', () => {
		...
		cy.get('[data-test-id=virtuoso-item-list] [cy-test=beacons-row]').eq(0).realClick();
		...
		cy.get('[cy-test=beacon-display-name]')
			...
			.then((resultBeacon1) => {
				...
				cy.get('[data-test-id=virtuoso-item-list] [cy-test=beacons-row]').contains(newBeaconName).realClick();
				cy.clickMetaTab();
